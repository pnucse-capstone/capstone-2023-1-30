...
if os.path.isdir("../data/B_dataset") is False:
    os.mkdir("../data/B_dataset")
...
if len(first_file_names) == len(second_file_names):
    for i in range(len(first_file_names)):
        ...
        df1 = df1[['Date', 'Close', '등락률', 'PER', 'PBR']]
        df1.columns = ['Date', 'Close', '등락률', 'PER', 'PBR']
        ...
        # 결과 출력
        merged_df_sorted.to_csv(f'../data/B_dataset/{first_file_names[i]}', index=False)